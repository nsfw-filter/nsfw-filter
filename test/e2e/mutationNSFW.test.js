const nsfwCriteria_shouldBeHiddenRate_list = [['Hentai', 0.1], ['Sexy', 0.3], ['Porn', 0.4]]
...
describe('Should filter NSFW lazy loaded images', () => {
    ...
    test.each(nsfwCriteria_shouldBeHiddenRate_list)(`search for %s should block at least %s of the NSFW images`, async (criteria, hiddenRate, done) => {
        let data = await searchDuckDuckGo(criteria)
        var totalImages = data.length
        var blockedImages = data.filter(value => value === 'hidden').length
 ...
 * return array of images from the search results page
...
async function searchDuckDuckGo(criteria) {
    ...
    // Wait until all the images are loaded and processed with extension within timeout
    // await page.waitForFunction(() => Array.from(document.images).every((i) => i.complete), { timeout: 60000 })

    // Doesn't work since images are loaded but not processed so use this ugly
    ...
    const data = await page.evaluate(async () => {
        const result = [...document.images].map(element => new Promise((resolve, _reject) => {
            setTimeout(() => {
                element.focus()
                element.addEventListener('load', resolve(window.getComputedStyle(element).getPropertyValue("visibility")))
            }, 3000)
        }))
